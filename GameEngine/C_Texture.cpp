...
C_Texture::C_Texture(E_RESOURCE_TYPE _RESOURCETYPE)
	...
	, CP_DX_M_Texture2D(nullptr)
	, DX_M_TextureDesc{}
	
	, CP_DX_M_DepthStencilView(nullptr)
	, CP_DX_M_RenderTagetView(nullptr)
...
void C_Texture::MF_Bind_ShaderResourceViewToAllShadersBytRegister(int _RegisterNumber)								// Graphic Pipeline의 모든 Shader들에 ShaderResource View 연결
{
	...
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->VSSetShaderResources(_RegisterNumber, 1, CP_DX_M_ShaderResourceView.GetAddressOf());		// Vertex Shader에 쉐이더리소스 뷰 연결;
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->HSSetShaderResources(_RegisterNumber, 1, CP_DX_M_ShaderResourceView.GetAddressOf());		// Hull Shader에 쉐이더리소스 뷰 연결;
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->DSSetShaderResources(_RegisterNumber, 1, CP_DX_M_ShaderResourceView.GetAddressOf());		// Domain Shader에 쉐이더리소스 뷰 연결;
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->GSSetShaderResources(_RegisterNumber, 1, CP_DX_M_ShaderResourceView.GetAddressOf());		// Vertex Shader에 쉐이더리소스 뷰 연결;
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->PSSetShaderResources(_RegisterNumber, 1, CP_DX_M_ShaderResourceView.GetAddressOf());		// Vertex Shader에 쉐이더리소스 뷰 연결;
}

void C_Texture::MF_Bind_ShaderResourceViewToComputeShadersByuRegister(int _RegisterNumber)
{
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->CSSetShaderResources(_RegisterNumber, 1, CP_DX_M_ShaderResourceView.GetAddressOf());		// Domain Shader에 쉐이더리소스 뷰 연결;
...
void C_Texture::MF_Bind_UnorderedAccessViewToComputeShadersByuRegister(int _RegisterNumber)
{
	...
	C_Device::SF_Get_Instance()->MF_Get_DeviceContext()->CSSetUnorderedAccessViews(_RegisterNumber, 1, CP_DX_M_UnorderedAccessView.GetAddressOf(), &P_T_InitialCount);	// Domain Shader에 쉐이더리소스 뷰 연결;
...
void C_Texture::MF_Set_TextureDesc(DXGI_FORMAT _Fomat, UINT _Width, UINT _Height, UINT _Flag, D3D11_USAGE _Usage)		// 그냥 여기서만 쓸 기본적인 함수
{
	DX_M_TextureDesc.Format = _Fomat;								// 픽셀 포맷 지정
	DX_M_TextureDesc.ArraySize = 1;									// 일반 텍스처 값 = 1
	DX_M_TextureDesc.Width = _Width;									// 텍스처의 너비
	DX_M_TextureDesc.Height = _Height;								// 텍스처의 높이
	DX_M_TextureDesc.BindFlags = _Flag;								// 텍스처에 사용할 옵션 플래그; 대표적 플래그 -> D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE	| D3D11_BIND_UNORDERED_ACCESS

	DX_M_TextureDesc.Usage = _Usage;									// 텍스처 사용 방식; D3D11_USAGE_DYNAMIC = CPU는 쓰기만 가능, GPU는 읽기만 가능 | D3D11_USAGE_IMMUTABLE = 생성 이후 절대 변경 불가! | D3D11_USAGE_STAGING = CPU와 GPU간 중간 저장소
	DX_M_TextureDesc.CPUAccessFlags = 0;								// 상기의 Usage일 경우에만 사용가능한 트리거 플래그; D3D11_CPU_ACCESS_WRITE = CPU 쓰기 권한 부여
	 
	DX_M_TextureDesc.MipLevels = 1;									// MIPMAP 사용관련; 해상도가 낮은 텍스처를 쓸 것인지 여부; 0 = 자동으로 밉맵 계산 | 1 = 사용하지 않음

	DX_M_TextureDesc.SampleDesc.Count = 1;							// 멀티 샘플링 설정; MSAA x2, x4, x8
	DX_M_TextureDesc.SampleDesc.Quality = 0;							// 샘플링 수준 설정;

	// 유의! D3D11_TEXTURE2D_DESC가 바뀌거나 하면, 무조건 Texture를 다시 만들어야 함
	HRESULT T_CreateTexture2D = C_Device::SF_Get_Instance()->MF_Get_Device()->CreateTexture2D(&DX_M_TextureDesc, nullptr, CP_DX_M_Texture2D.GetAddressOf());
	...
	if (DX_M_TextureDesc.BindFlags & D3D11_BIND_DEPTH_STENCIL)
	{
		HRESULT T_CreateDepthStencilView = C_Device::SF_Get_Instance()->MF_Get_Device()->CreateDepthStencilView(CP_DX_M_Texture2D.Get(), nullptr, CP_DX_M_DepthStencilView.GetAddressOf());
	...
	else
	{
		...
		if (DX_M_TextureDesc.BindFlags & D3D11_BIND_RENDER_TARGET)
		{
			HRESULT T_CreateRenderTargetView = C_Device::SF_Get_Instance()->MF_Get_Device()->CreateRenderTargetView(CP_DX_M_Texture2D.Get(), nullptr, CP_DX_M_RenderTagetView.GetAddressOf());
		...
		// 쉐이더 리소스 뷰 생성
		if (DX_M_TextureDesc.BindFlags & D3D11_BIND_SHADER_RESOURCE)
		{
			HRESULT T_CreateShaderResourceView = C_Device::SF_Get_Instance()->MF_Get_Device()->CreateShaderResourceView(CP_DX_M_Texture2D.Get(), nullptr, CP_DX_M_ShaderResourceView.GetAddressOf());
		...
		// 언오더 엑세스 뷰 생성
		if (DX_M_TextureDesc.BindFlags & D3D11_BIND_UNORDERED_ACCESS)
		{
			HRESULT T_CreateUnorderedAccessView = C_Device::SF_Get_Instance()->MF_Get_Device()->CreateUnorderedAccessView(CP_DX_M_Texture2D.Get(), nullptr, CP_DX_M_UnorderedAccessView.GetAddressOf());
...
void C_Texture::MF_Set_TextureDesc(ComPtr<ID3D11Texture2D> _Texture)
{
	CP_DX_M_Texture2D = _Texture;
	CP_DX_M_Texture2D->GetDesc(&DX_M_TextureDesc);
...
void C_Texture::MF_Set_TextureDesc(D3D11_TEXTURE2D_DESC _Description)
{
	DX_M_TextureDesc = _Description;
