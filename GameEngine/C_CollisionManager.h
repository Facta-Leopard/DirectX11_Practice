...
protected:
    ...
    unordered_map<ULARGE_INTEGER,bool>              STL_M_SDKIsCollision;                          // unordered_map<ULARGE_INTEGER,bool>; 초반 버킷을 정하는 방식이며, 향후 확장성까지 고려하여 기존의 map보다는 더 순회며 조회가 더 빠를 것으로 예상되므로 STL 변경
    UINT                                            SDK_M_BucketSize;                              // UINT; unordered_map 속도 증가를 위한 캐싱용
...
public:
    ...
    bool MF_Check_SAT(C_Collider2D _ColliderA, C_Collider2D _ColliderB);
};
